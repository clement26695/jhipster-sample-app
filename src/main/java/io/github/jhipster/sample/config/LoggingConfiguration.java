...
public class LoggingConfiguration {
    ...
    public LoggingConfiguration(
        ...
        if (this.jHipsterProperties.getLogging().isUseJsonFormat()
            || this.jHipsterProperties.getLogging().getLogstash().isEnabled()) {
    ...
    private void addLogstashTcpSocketAppender(LoggerContext context) {
        ...
        asyncLogstashAppender.setQueueSize(
            this.jHipsterProperties.getLogging().getLogstash().getQueueSize()
        );
    ...
    private GlobalCustomFieldsJsonProvider<
        ILoggingEvent
    > customFieldsJsonProvider() {
        final GlobalCustomFieldsJsonProvider<
            ILoggingEvent
        > customFieldsJsonProvider = new GlobalCustomFieldsJsonProvider<>();
        ...
        context.getLoggerList()
            .forEach(
                logger -> {
                    ...
                    > it = logger.iteratorForAppenders(); it.hasNext();) {
                        Appender<ILoggingEvent> appender = it.next();
                        if (!appender.getName()
                            .equals(ASYNC_LOGSTASH_APPENDER_NAME)
                            && !(appender.getName()
                                .equals(CONSOLE_APPENDER_NAME)
                                && this.jHipsterProperties
                                    .getLogging()
                                    .isUseJsonFormat())) {
                            log.debug(
                                "Filter metrics logs from the {} appender",
                                appender.getName()
                            );
